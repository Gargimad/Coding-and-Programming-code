...
#Bookmarks-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    def displayBusinesses(self, sub_id, sub_name, city_id):
        ...
        self.city_mb = tk.Menubutton(sortingBtnFrame, text="Select GA City ⏷", 
                               bg=colors["button"], fg=colors["button_text"], font=("Georgia", 10), 
        ...
        self.citySelectDrpMenu = tk.Menu(self.city_mb, tearoff=0, bg=colors["button"], fg=colors["button_text"])
        ...
        tk.Button(sortingBtnFrame, text="Highest Ratings", bg=colors["button"], fg=colors["button_text"], 
                  font=("Georgia", 10), width=15, relief="flat", cursor="hand2",
                  command=lambda: self.sortByRatings(businesses)).pack(side="left", padx=5)
        
        tk.Button(sortingBtnFrame, text="Most Reviewed", bg=colors["button"], fg=colors["button_text"], 
                  font=("Georgia", 10), width=15, relief="flat", cursor="hand2",
...
#Selection of communities-------------------------------------------------------------------------------------------------------------------------------------
    def displayCities(self):
        ...
        self.citySelectDrpMenu.configure(bg=colors["button"], fg=colors["button_text"])
    ...
    def renderBusinessCards(self, businesses):
        ...
        for biz_id, biz_name, rating, review_count, description, website_link in businesses:
            ...
            tk.Label(bizCard, text=f"{description}", font=("Georgia", 10), bg=colors["card"], fg=colors["card_text"],
            ...
            if self.userEmail:
                ...
                bmColor = colors["bookmark_added"] if bmSaved else colors["bookmark_remove"]
                
                bmBtn = tk.Button(bizCard, text=bmTxt, bg=bmColor, fg=colors["button_text"], 
                                 relief="flat", padx=10, cursor="hand2")
    ...
    def homeUi(self):
        ...
        try:
            ...
            tk.Label(self.mainPageFrame, text="[Logo Error]", bg=colors["bg"], fg=colors["text"], font=("Georgia", 20)).grid(row=0, column=0)
        ...
        tk.Label(self.mainPageFrame, text="Local business now becomes just a PIBBIT away",
                 font=("Georgia", 25), fg="#6E2F20" if not self.darkMode else "#FFA07A", 
...
#Coupons and Deals--------------------------------------------------------------------------------------------------------------------------------
    def showBusinessCoupons(self, biz_id, biz_name):
        ...
        tk.Label(popup, text=f"Coupons for {biz_name}", 
                font=("Georgia", 14, "bold"), bg=colors["card"], fg=colors["card_text"], pady=10).pack()
        ...
        if not coupons:
            tk.Label(popup, text="No active coupons for this business.", 
                    font=("Georgia", 11), bg=colors["card"], fg=colors["card_text"]).pack(pady=50)
        else:
            for title, description, coupon_code in coupons:
                ...
                tk.Label(f, text=description, font=("Georgia", 10), 
                        bg=f.cget('bg'), fg=colors["card_text"], wraplength=400).pack(anchor="w")
    ...
    def showAllCouponsPage(self):
        ...
        # 2. Setup Canvas with Dynamic Width
        ...
        for biz_name, title, desc, code in all_coupons:
            ...
            tk.Label(card, text=biz_name, font=("Georgia", 18, "bold"), 
                    bg=colors["card"], fg=colors["card_text"]).pack(anchor="w")
            ...
            tk.Label(card, text=desc, font=("Georgia", 11), 
                    bg=colors["card"], fg=colors["card_text"], wraplength=800, justify="left").pack(anchor="w", pady=5)
...
#Handling Captcha and bot safety after opening login and signup page--------------------------------------------------------------------------------------------------
    ...
    def applyTheme(self):
        ...
        if hasattr(self, 'mb'):
            self.mb.configure(bg=colors["button"], fg=colors["button_text"])
        
        if hasattr(self, 'coupsNDeals'):
            self.coupsNDeals.configure(bg=colors["button"], fg=colors["button_text"])
        
        if hasattr(self, 'helpBtn'):
            self.helpBtn.configure(bg=colors["button"], fg=colors["button_text"])
        
        if hasattr(self, 'bookmarks'):
            self.bookmarks.configure(bg=colors["button"], fg=colors["button_text"])
        ...
        if hasattr(self, 'signup_btn'):
            self.signup_btn.configure(bg=colors["button"], fg=colors["button_text"])
        
        if hasattr(self, 'login_btn'):
            self.login_btn.configure(bg=colors["button"], fg=colors["button_text"])
        ...
        if hasattr(self, 'mainPageFrame') and self.mainPageFrame.winfo_ismapped():
            ...
            for widget in self.mainPageFrame.winfo_children():
                if isinstance(widget, tk.Label):
                    ...
                    elif widget.cget('text') == "Local business now becomes just a PIBBIT away":
                        widget.configure(bg=colors["bg"], fg="#FFA07A" if self.darkMode else "#6E2F20")
                    else:
                        try:
                    ...
                    for child in card.winfo_children():
                        if isinstance(child, tk.Label):
                            if "★" in child.cget('text') or "☆" in child.cget('text'):
                                child.configure(bg=colors["card"], fg=colors["rating_stars"])
                            elif child.cget('text') == biz_name:  # Business name
                                child.configure(bg=colors["card"], fg=colors["card_text"])
                            else:
                                child.configure(bg=colors["card"], fg=colors["card_text"])
    ...
    def toggleDarkMode(self):
        ...
        if hasattr(self, 'cardsFrame') and self.cardsFrame.winfo_ismapped():
            # Refresh the business cards with new theme
            pass